    ...
    async function pipeline(userText, { retry = false, isSwipe = false } = {}) {
        ...
        try {
            ...
            if (!aHit || !bHit) {
                ...
                if (!aHit) jobs.push(
                    callOnce(S.fastModel, sub(S.promptA), [matA])
                        .then(t => { notesA = t; cacheAB.presetHash = presetHash; cacheAB.notesA = t; })
                );
                if (!bHit) jobs.push(
                    callOnce(S.fastModel, sub(S.promptB), [matB])
                        .then(t => {
                            ...
                            if (wiOk) { cacheAB.charHash = charHash; cacheAB.notesB = t; }
            ...
            if (cacheC.inputKey === cInputKey && cacheC.notes) {
                ...
                notesC = await callOnceRetry(S.model, sub(S.promptC), [
                    ...cMaterials,
                    ...history,
            ...
            // C 格式矫正：漂移时（不以【开头）打上标记供下游识别
            if (notesC && !notesC.trim().startsWith('【')) {
                const hasThink = notesC.includes('协议思考') || notesC.includes('剧情') || notesC.includes('规划');
                if (hasThink) {
                    notesC = `【协议思考】\n${notesC.trim()}`;
                    log('C 格式矫正：漂移输出已打标');
                }
            }
            log('C 思考中枢完成', notesC.length, '字');
            ...
            // 统一 push 新楼（retry/正常回合；swipe 上面已处理跳过）
            if (!isSwipe) {
                message = {
                    name: ctx.name2,
            ...
            const depthParts = inserts.length
                ...
                : '';
            // D 材料预算控制：总材料 ≤ 12000 字（超出时裁 ctxBlock 尾部）
            const D_BUDGET = 12000;
            const trimMat = (label, text, budgetLeft) => {
                if (text.length <= budgetLeft) return { text, used: text.length };
                const trimmed = text.slice(0, Math.max(500, budgetLeft));
                log(`D材料裁剪: ${label} ${text.length}→${trimmed.length}字`);
                return { text: trimmed, used: trimmed.length };
            };
            let dBudget = D_BUDGET;
            const dParts = [];
            const addMat = (label, text) => {
                if (!text) return;
                const { text: t, used } = trimMat(label, text, dBudget);
                if (t) { dParts.push({ role: 'system', content: t }); dBudget -= used; }
            };
            ...
            if (postOn) {
                ...
                try {
                    problems = await callOnce(S.fastModel, sub(S.promptE), [
                        { role: 'system', content: `【预设要求清单】\n${notesA}` },
                        { role: 'system', content: `【角色卡要求清单】\n${notesB}` },
                        { role: 'system', content: `【协议思考与创作蓝图】\n${notesC}` },
                        { role: 'user', content: `【草稿】\n${draftText}` },
                    ], { timeoutMs: 120000 });
                    log('E 审查完成', problems.length, '字');
                } catch (e) {
                    if (e?.name === 'AbortError') throw e;   // 用户中止：终止全链
                    log('E 审查失败（视为通过）', e?.message);
                    problems = '';
                }
            ...
            const mdH = meta();
            const hPromise = (postOn && mdH) ? callOnce(S.fastModel, sub(S.promptH), [{
                role: 'user',
                content: [
                    `【剧情摘要】\n${mdH?.summary || '（暂无）'}`,
                    `【本回合最终回复】\n${finalContent.slice(0, 4000)}`,
                ].filter(Boolean).join('\n\n'),
            }], { timeoutMs: 120000 }).catch(e => {
                if (e?.name === 'AbortError') throw e;
            ...
            if (postOn && mdG && !retry && !isSwipe) {
                ...
                try {
                    ...
                    const hOut = (await callOnce(S.fastModel, sub(S.promptG) + (needPhase ? '\n（本回合需要执行任务二·阶段总结）' : ''), [{
                        role: 'user',
                        content: [
                            `【本回合用户输入】\n${userText}`,
                            `【本回合最终回复】\n${finalContent.slice(0, 4000)}`,
            ...
            if (message && message.mes && /Agent 创作中|审查中|Swipe 生成中/.test(message.mes)) {
                // mes 是占位符——不是真实内容
...
<div class="dsrp-panel">
  <div class="inline-drawer">
    ...
    function addPanel() {
        ...
        // 挂载 Tester 到主面板内（Tester 自己的面板不再独立注入）
        const testerMount = $('#dsrp_tester_mount');
        if (testerMount.length && !testerMount.children().length) {
            const testerPanel = $('#dsrt_log').closest('.dsrt-panel');
            if (testerPanel.length) {
                testerMount.append(testerPanel);
                log('Tester 已挂载到主面板');
            }
        }
